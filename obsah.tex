...
Druhým typem IDS je detekce založená na pravidlech. Tato pravidla jsou pevně daná a systém pouze porovnává síťový provoz s~danými pravidly. Nevýhodou takového systému je neschopnost detekce neznámých útoků. Některé IDS kombinují oba přístupy a tvoří tak hybdridní IDS, který je založen jak na pravidlech, tak na automatické detekci anomálií.
...
Modulární systém navíc dovoluje přidávat a odebírat jednotlivé moduly za běhu, což je v~produkčním prostředí jedna ze základních vlastnostní kvalitního monitorovacího systému. Pro tyto účely slouží nástroj nazvaný NEMEA Supervisor.
...
K~vytvoření SPA a zejména uživatelské části existuje několik významných systémů. V~této části si tyto technologie představíme a porovnáme mezi sebou. Nejdůležitější částí je JavaScriptový framework, který bude nejčastěji interagovat s~uživatelem. Další nedílnou součástí je knihovna pro HTML/CSS, která bude definovat vzhled aplikace.

\subsection{JavaScriptové frameworky}

Bylo vybráno 5 JavaScript frameworků, které napomáhají k~tvorbě SPA. Kritéria užšího výběru frameworků byla zejména následující:
...
Společnost Google disponuje vizuálním jazykem pro všechny své produkty nazvaný Material Design\footnote{\url{https://www.google.com/design/spec/material-design/introduction.html}}. Ten je založený na tzv. \uv{kartičkách}, které lze skládat na sebe, řadit, organizovat a upravovat. Jak již název frameworku napovídá je tento vizuální jazyk použit skrz všechny dostupné komponenty.
...
C++ obálka frameworku Mongoose a NodeJS jsou několikanásobně rychlejší než framework Flask. Tato rychlost je zejména díky použitému jazyku C++. Ten je použit i v~případě NodeJS, kde většina komponent je právě napsána v~C++. 
...
\begin{description}
    ...
    \item \texttt{/top}\\
        Slouží pro box typu \uv{TOP}. Ten vybírá z~databáze událost, která obsahuje nejvyšší číslo \texttt{FlowCount} u~každé z~kategorií v~určitém časovém okně.
...
Drill-down analýza je realizována jako odkaz v~jednotlivých sloupcích/oddílech grafu nebo jako tlačítko (v~případě boxu typu \uv{top}), které odkazuje přímo na danou událost. Při kliknutí na danou část grafu nebo tlačíko je uživatel přesunut do druhé části NEMEA Dashboard nazvanou \uv{Events}.
